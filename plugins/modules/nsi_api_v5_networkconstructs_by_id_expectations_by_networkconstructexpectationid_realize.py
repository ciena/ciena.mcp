...
DOCUMENTATION = """
module: nsi_api_v5_networkconstructs_by_id_expectations_by_networkconstructexpectationid_realize
short_description: Handle resource of type nsi_api_v5_networkconstructs_by_id_expectations_by_networkconstructexpectationid_realize
description: Handle resource of type nsi_api_v5_networkconstructs_by_id_expectations_by_networkconstructexpectationid_realize
options:
  id:
    description:
    - Identifier of the Network Construct under which the expectation is to be realized
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v5/networkConstructs/{id}/expectations/{networkConstructExpectationId}/realize".format(
        **params
    )
...
async def _post(params, session):
    accepted_fields = ["id", "networkConstructExpectationId"]
    spec = {}
    for i in accepted_fields:
        if params[i] is not None:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v5/networkConstructs/{id}/expectations/{networkConstructExpectationId}/realize".format(
        **params
    ) + gen_args(
