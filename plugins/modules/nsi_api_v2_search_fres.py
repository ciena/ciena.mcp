...
DOCUMENTATION = """
module: nsi_api_v2_search_fres
short_description: Handle resource of type nsi_api_v2_search_fres
description: Handle resource of type nsi_api_v2_search_fres
options:
  active:
    choices:
    - 'false'
  ...
  freType:
    description:
    - 'FRE types in comma separated list The allowed values are: explicitRoute, cascadedExplicitRoute,
      explicitRouteGroup, snc, sncGroup, explicitRouteInstance'
    - Used by I(state=['get', 'post'])
    type: str
  gneSubnetName:
    description:
    - (Optional) GNE Subnet name of the FRES
    - Used by I(state=['get', 'post'])
    type: str
  group:
    choices:
  ...
  networkConstruct.id:
    description:
    - Comma separated Network Construct identifier
    - Used by I(state=['get', 'post'])
    type: str
  networkConstruct.id.operator:
    ...
    - or
    description:
    - (Optional) The logical operator to use when searching Network Construct identifier
    - Used by I(state=['get', 'post'])
    type: str
  networkRole:
    choices:
  ...
  ovpnIds:
    description:
    - (Optional) List of comma separated ovpnIds (0-255)
    - Used by I(state=['get', 'post'])
    type: str
  partitionFreIds:
    description:
  ...
  serviceClass:
    ...
    - '(Optional) Allow filtering on the FRE service class. This parameter accepts
      ...
      Fiber, SNC , OSRP Link, OSRP Line, SNCP, ICL, Ring'
    - Used by I(state=['get', 'post'])
    type: str
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v2/search/fres".format(**params)
...
async def _get(params, session):
    ...
    for i in accepted_fields:
        if params[i] is not None:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v2/search/fres".format(**params) + gen_args(
