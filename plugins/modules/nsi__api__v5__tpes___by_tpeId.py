...
DOCUMENTATION = """
module: nsi__api__v5__tpes___by_tpeId
short_description: Handle resource of type nsi__api__v5__tpes___by_tpeId
description: Handle resource of type nsi__api__v5__tpes___by_tpeId
options:
  data:
    description:
...
def prepare_argument_spec():
    ...
    argument_spec["tpeId"] = {"type": "str", "operationIds": ["put"]}
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v5/tpes/{tpeId}".format(**params)
...
async def _put(params, session):
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v5/tpes/{tpeId}".format(**params)
