...
DOCUMENTATION = """
module: ifd_api_v1_serviceintents_by_id_undeploy
short_description: Handle resource of type ifd_api_v1_serviceintents_by_id_undeploy
description: Handle resource of type ifd_api_v1_serviceintents_by_id_undeploy
options:
  id:
    description:
  ...
  ignoreClientServiceDependencies:
    description:
    - (Optional. Demo Only - not officially supported) Bypasses service provisioning
      client service validation if set to true
    - Used by I(state=['post'])
    type: bool
  state:
    choices:
...
- python >= 3.6
"""
IN_QUERY_PARAMETER = ["ignoreClientServiceDependencies"]
...
def prepare_argument_spec():
    ...
    argument_spec["ignoreClientServiceDependencies"] = {
        "type": "bool",
        "operationIds": ["post"],
    }
...
def url(params):
    return "https://{mcp_hostname}/ifd/api/v1/serviceIntents/{id}/undeploy".format(
        **params
    )
...
async def _post(params, session):
    accepted_fields = ["id", "ignoreClientServiceDependencies"]
    spec = {}
    for i in accepted_fields:
        if params[i] is not None:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/ifd/api/v1/serviceIntents/{id}/undeploy".format(
        **params
