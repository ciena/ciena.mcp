...
DOCUMENTATION = """
module: nsi_api_v3_networkconstructs_by_id_expectations_by_expectationid
short_description: Handle resource of type nsi_api_v3_networkconstructs_by_id_expectations_by_expectationid
description: Handle resource of type nsi_api_v3_networkconstructs_by_id_expectations_by_expectationid
options:
  expectationId:
    description:
...
def prepare_argument_spec():
    ...
    argument_spec["id"] = {"type": "str", "operationIds": ["patch"]}
    argument_spec["expectationId"] = {"type": "str", "operationIds": ["patch"]}
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v3/networkConstructs/{id}/expectations/{expectationId}".format(
        **params
    )
...
async def _patch(params, session):
    accepted_fields = ["expectationId", "id", "operations"]
    spec = {}
    for i in accepted_fields:
        if params[i] is not None:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v3/networkConstructs/{id}/expectations/{expectationId}".format(
        **params
    ) + gen_args(
