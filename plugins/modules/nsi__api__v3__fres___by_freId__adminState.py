...
DOCUMENTATION = """
module: nsi__api__v3__fres___by_freId__adminState
short_description: Handle resource of type nsi__api__v3__fres___by_freId__adminState
description: Handle resource of type nsi__api__v3__fres___by_freId__adminState
options:
  adminStateValue:
    description:
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v3/fres/{freId}/adminState".format(**params)
...
async def _put(params, session):
    accepted_fields = ["adminStateValue"]
    spec = {}
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v3/fres/{freId}/adminState".format(**params)
