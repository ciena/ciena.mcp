...
DOCUMENTATION = """
module: nsi__api__v3__fres__deploymentState
short_description: Handle resource of type nsi__api__v3__fres__deploymentState
description: Handle resource of type nsi__api__v3__fres__deploymentState
options:
  ...
  freId:
    description:
    - Top-level FRE Identifier. Mutually exclusive with freExpectations.serviceIntent.id
    type: str
  state:
    choices:
    - put
...
- python >= 3.6
"""
IN_QUERY_PARAMETER = ["freExpectations.serviceIntent.id", "freId"]
...
def prepare_argument_spec():
    ...
    argument_spec["deploymentStateValue"] = {"type": "str", "operationIds": ["put"]}
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v3/fres/deploymentState".format(**params)
...
async def _put(params, session):
    accepted_fields = ["deploymentStateValue"]
    spec = {}
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v3/fres/deploymentState".format(**params)
