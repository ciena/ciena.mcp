...
DOCUMENTATION = """
module: nsi__api__v4__tpes___by_tpeId
short_description: Handle resource of type nsi__api__v4__tpes___by_tpeId
description: Handle resource of type nsi__api__v4__tpes___by_tpeId
options:
  data:
    description:
...
def url(params):
    return "https://{mcp_hostname}/nsi/api/v4/tpes/{tpeId}".format(**params)
...
async def _get(params, session):
    _url = "https://{mcp_hostname}/nsi/api/v4/tpes/{tpeId}".format(**params) + gen_args(
...
async def _put(params, session):
    ...
    for i in accepted_fields:
        if params[i]:
            spec[i] = params[i]
    _url = "https://{mcp_hostname}/nsi/api/v4/tpes/{tpeId}".format(**params)
