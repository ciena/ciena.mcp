...
DOCUMENTATION = """
module: type_artifacts__realm__validate
short_description: Handle resource of type type_artifacts__realm__validate
description: Handle resource of type type_artifacts__realm__validate
options:
  state:
    choices:
    - post
...
def url(params):
    return "https://{mcp_hostname}/bpocore/market/api/v1/type-artifacts/realm/validate".format(
        **params
    )
...
async def _post(params, session):
    _url = "https://{mcp_hostname}/bpocore/market/api/v1/type-artifacts/realm/validate".format(
        **params
    ) + gen_args(
